...
The Windows host runs: **STT server** (faster-whisper on RTX 4090), **TTS server** (Kokoro), and the **orchestrator** (mic capture + F8 hotkey + glue). The VM runs the **Claude wrapper daemon** and the **`speak` CLI**. Two HTTP wires connect them: host → VM `POST /ask`, VM → host `POST /speak`.
...
Press **F8** on the Windows side, speak within five seconds, release — you should hear a reply through the host's speakers.
...
| `VOICE_HOTKEY` | `f8` | Push-to-talk key |

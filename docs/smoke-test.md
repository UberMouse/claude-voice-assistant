...
Manual tests (run with focus on a window that won't swallow Lshift+F3):

1. **Q&A**: Press Lshift+F3. Within 5s say "what's the capital of France". Wait. You should hear "Paris" (or a brief similar reply).
2. **Note**: Press Lshift+F3. Say "save a note that I need to email Sam tomorrow". Confirm a file appears in `~/voice-assistant/notes/`.
3. **Recall**: Press Lshift+F3. Say "what notes did I save today". Should describe the note.
